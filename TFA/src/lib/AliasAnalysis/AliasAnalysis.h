...
typedef struct AliasContext {
    ...
    map<AliasNode*, AliasNode*> ToNodeMap; //One node points to which node
    map<AliasNode*, AliasNode*> FromNodeMap; //One node is pointed to which node
...
//When precise alias failed, we use the following type-target data-flow analysis
